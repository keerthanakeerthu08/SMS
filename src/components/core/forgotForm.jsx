...
class ForgotForm extends Component {
  ...
  schema = {
    uid: Joi.string().required().label("Email/User ID"),
    otp: Joi.string().required().label("OTP"),
    newPsw: Joi.string().required().label("New Password"),
    confirmPsw: Joi.string().required().label("Confirm Password"),
  ...
  updatePassword = async () => {
    ...
    try {
      ...
      if (statusCode) {
        await this.setState({ isOTPValid: true, isFeildVisible: true, })
        ToastService.Toast("Password changed successfully", "default");
          ...
          <Col sm="12" >
            <div className="flexdiv d-flex align-items-center flex-column justify-content-center" style={{ minHeight: '100vh' }}>
              <div className="d-flex align-items-center flex-column justify-content-center bg-white customdiv" style={{ padding: '20px' }}>
                ...
                <label>Enter Email Id/User ID to change your password</label>
                <Form getApi={this.setFormApi} onSubmit={this.onSubmit} style={{ width: '100%' }} >
                  <Input
                    field="uid" label="Email Id/User ID"
                  ...
                  <FormGroup className="text-right" >
                    <Button color="primary" type="button" onClick={this.goHome}>
                     Login
