...
class ClientEdit extends Component {
  ...
  shortEntitySchema = {
    ...
    logo: Joi.string().uri().trim().required().label("Entity Logo"),
  ...
  schema = {
    ...
    branchId: Joi.any().required().label("Branch id"),
    branchName: Joi.string().required().label("Branch Name"),
    addremail: Joi.string().email().required().label("Email "),
    // mobile: Joi.string().min(10).max(10).regex(/^\(?([0-9]{3})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})$/).required().label("Mobile Number"),
    mobile: Joi.string().regex(/^\+[1-9]{1}[0-9]{3,14}$/).required().label("Mobile Number"),
    type: Joi.string().required().label("Type"),
    code: Joi.string().alphanum().min(3).max(30).required().label("Code"),
    internalCode: Joi.any().optional(),
    name: Joi.string().required().label('Name'),
    shortName: Joi.string().required().label("Short Name"),
    logo: Joi.string().uri().trim().optional().label(" Logo"),
    icon: Joi.string().uri().trim().optional().label(" Icon"),
  ...
  helper = {
    mobile: <Fragment>Mobile Number(International Format) for communication. <b>Eg.</b><i><strong>+919090909090</strong></i></Fragment>
  }
  ...
  //   const { from, to } = date;
  //   if (from && to) {
  //     data.entity[i][feild] = { from: moment(from).format("YYYY-MM-DD"), to: moment(to).format("YYYY-MM-DD") };
  //     // data.entity[i][feild] = { from: from._d.toISOString().slice(0, 10), to: to._d.toISOString().slice(0, 10) };
  ...
  //   }
  // }
  dateValue = async (date,i, field) => { // Get dates from the data range picker
    const data = this.formApi.getState().values; 
    let { from, to } = date; 
    if (!moment(to).isValid()) to = from;
    data.entity[i][field] = { from: moment(from).format("YYYY-MM-DD"), to: moment(to).format("YYYY-MM-DD") };
  ...
  saveClient = async (data) => {
    ...
    if (statusCode !== 1) return alert(message);
    this.redirectTo();
  }
  ...
  handleEntityImage = async (i, { target: { files } }) => {
    ...
    let entityLogo = await FileUpload(files[0]).then((res) => res.data.name)
    await entityImages.push({ location: i, logo: entityLogo })
    await this.setState({ entityImages });
    const values = this.formApi.getState().values;
    values.entity[i].logo = entityLogo
  ...
  render() {
    ...
    const { props, session, match: { params: { id } } } = this.props;
                                ...
                                <Row>
                                  ...
                                  <Col sm={12} md={4}>
                                    <Input
                                      ...
                                      validateOnBlur validate={e => this.validateProperty('mobile', e)} helper={this.helper.mobile}
                                    />
                                  </Col>
