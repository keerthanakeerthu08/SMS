...
export default class ClientList extends Component {
    ...
    async exceltable(format) {
        ...
        switch (format) {
            case "selecteduserxls":
                if (format === 'selecteduserxls' && exportData !== undefined && exportData.length > 0) {

                    filterData = _.map(exportData, o => _.omit(o, ['type', 'row']));
                    // if(exportData.type=="client")
                    d = this.downloadxls(filterData)
            ...
            case "alluserxls":
                if (format === 'alluserxls' && data !== undefined && data.length > 0){
                    filterData = _.map(data, o => _.omit(o, ['type', 'row']));
                    d = this.downloadxls(filterData)
                }else
                    return ToastService.Toast("Please select the columns to download", "default");
                break;
    ...
    clientLinkFormatter = (cell, row, rowIndex, formatExtraData) => {
        ...
        switch (type) {
            case "client":
                links.push(<NavLink key={rowIndex} to={`${prefixUrl}/entity/list`} title="Click to See Entity List" onClick={() => this.refreshTable()} className='cursor badge badge-primary'>{cell}</NavLink>)
                break;
            case "entity":
                links.push(<NavLink key={rowIndex} to={`/${prefixUrl}/branch/list`} title="Click to See Branch List" onClick={() => this.refreshTable()} className='cursor badge badge-primary'>{cell}</NavLink>)
                break;
            case "branch":
                links.push(<NavLink key={rowIndex} to={`/${prefixUrl}/department/list`} title="Click to See Department List" onClick={() => this.refreshTable()} className='cursor badge badge-primary'>{cell}</NavLink>)
                break;
    ...
    actionsFormatter = (cell, row, rowIndex, formatExtraData) => {
        ...
        links.push(<div key={'delete' + rowIndex} onClick={() => confirm("Are you sure? You want to block this client.", () => this.deleteFun(row, 'blocked'))} className='badge badge-danger' > Block</div>)
        rightsData && rightsData[_form] && rightsData[_form].edit.value && row.row.name.status !== 'active' &&
          links.push(<div key={'delete' + rowIndex} onClick={() => confirm("Are you sure? You want to unblock this client.", () => this.deleteFun(row, 'active'))} className='badge badge-danger'>UnBlock</div>)
            ...
            <React.Fragment >
                {!isPageLoading && <React.Fragment>
                    ...
                    <div className="d-md-flex align-items-md-center justify-content-md-between">
                            ...
                            {rightsData && rightsData[_form] && rightsData[_form].create.value && type === 'client' &&
                                ...
                                <UncontrolledButtonDropdown >
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownItem className="a" onClick={() => { this.exceltable('alluserxls') }}>All {labels[type][0]} List</DropdownItem>
                                        <DropdownItem className="a" onClick={() => { this.exceltable('selecteduserxls') }} >Selected {labels[type][0]} List</DropdownItem>
                        ...
                        {
                          this.state.exportData && this.state.exportData.length > 0 &&
                          <DropdownItem>
                          <CSVLink
                        ...
                        <div className="alert alert-info alert-sm">
                            ...
                            {colKeys.map((k, i) => {
                                ...
                                return <div key={`toggle_${k}`} className="custom-control custom-control-inline col-6 col-md-2 custom-checkbox">
                                    ...
                                    <label className="custom-control-label clientHideColLabel" htmlFor={`toggle_${k}`}>{colDef[k]['desc'] ? colDef[k]['desc'] : colDef[k]['text']}</label>
                                </div>
                            })}
                        </div>
                    </Collapse>
