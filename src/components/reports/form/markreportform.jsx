...
export default class MarksReport extends Component {
  ...
  renderReportsForm(data, formData, studentId) {
    ...
    return <MarkReportList form={markSystem} data={data} formData={formData} rightsData={rightsData} studentUid={studentId} />
  }
  ...
  render() {
    const { clientIds, entityIds, branchIds, departmentIds, batchIds, studentList, formdata, isTableLoading, isClient, isEntity, isBranch, isDepartment, tableData, isBatch, studentId, data: { userType, uid } } = this.state;
    const { type } = this.props

    return (
      <Fragment >
        <div className="d-md-flex align-items-md-center justify-content-md-between">
          <h5 className="pg-title"> {type} Report</h5>
        ...
        {
          !isTableLoading && formdata && (studentId || uid) &&
