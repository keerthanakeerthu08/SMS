...
export default class HomeworkReport extends Component {
    ...
    renderReportsForm(type, data) {
        const {data:{userType}} = this.state;
        const { rightsData } = this.props;
        return <ReportList type={type} data={data} rightsData={rightsData} userType={userType} />
    }
    ...
    async  viewReport(data) {
        ...
        for (let item of data) {
            if (item.homeWorkReport) {
                ...
                homeworkView.push({ "studentId": studentId, "name": name, "topic": topic, "subject": subject, "status": status, "remarks": remarks,"createdDate":date })
