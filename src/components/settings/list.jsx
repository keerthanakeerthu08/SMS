...
export default class SettingsList extends Component {
        ...
        if (formType === 'department' || formType === 'batch' || formType === 'boardtype' || formType === 'religion' || formType === 'title') {
            ...
            await this.setState({
                columns: [
                    ...
                    { dataField: 'shortName', text: 'Short Name',sort:true },
                    { dataField: 'displayName', text: 'Name', filter: textFilter(),sort:true },
            ...
            await this.setState({
                columns: [
                    { dataField: 'code', text: 'Code',sort:true },
                    { dataField: 'shortName', text: 'Name',sort:true },
                    { dataField: 'name', text: 'Full Name', filter: textFilter(),sort:true },
                    { dataField: 'actions', isDummyField: true, text: "Actions", formatter: this.actionsFormatter }
                ]
            })
            ...
            await this.setState({
                columns: [
                    { dataField: 'country', text: 'Country Code',sort:true },
                    { dataField: 'code', text: 'Id',sort:true },
                    { dataField: 'name', text: 'State Name', filter: textFilter(),sort:true },
                    { dataField: 'actions', isDummyField: true, text: "Actions", formatter: this.actionsFormatter }
                ]
            })
        }
    ...
    actionsFormatter = (cell, row, rowIndex, formatExtraData) => {
        ...
        rightsData && rightsData[_form] && rightsData[_form].delete.value &&
            links.push(<div onClick={() => confirm("Are you sure? You want to delete this.", () => this.actionSetting(row ))} className='badge badge-danger'>Delete</div>)
