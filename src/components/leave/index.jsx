...
export default class leave extends Component {
  ...
  renderLeaveForm(data) {
    ...
    const { rightsData, data: sessionData,data:{userType} } = this.state;
    return <LeaveList data={data} rightsData={rightsData} sessionData={sessionData} refreshTable={this.onSubmit}   userType={userType} />
  }
                ...
                <Fragment>
                  ...
                  <Container fluid>
                    <div className="d-md-flex align-items-md-center justify-content-md-between">
                      <h5 className="pg-title">Leave</h5>
                      <div  >
                        {rightsData && rightsData[_form] && rightsData[_form].create.value &&
                          <NavLink className="btn btn-primary btn-sm" to={`add/${leaveType}`}>+ Add {leaveType} </NavLink>
                        }
                      </div>
                    </div>
