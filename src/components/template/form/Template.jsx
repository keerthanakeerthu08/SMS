    ...
    onSubmit = async () => {
        ...
        if (type === 'mail') {
            ...
            if (actionType === 'add') {
                ...
                if (config && config !== '') {
                    r = await SGTemplates(url, "POST", { "name": versions[0].name, "generation": "dynamic" }, config);
                    if (r && r.id) {
                                ...
                                <section>
                                    {formState.values.versions && formState.values.versions.map((v, i) =>
                                        <Scope scope={`versions[${i}]`} key={i}>
                                            <Row>
                                                <Col sm={12} md={3}>
                                                    <Input field="name" label="Template Name*" name="name" validateOnBlur validate={e => this.validateProperty('name', e)} />
                                                </Col>
