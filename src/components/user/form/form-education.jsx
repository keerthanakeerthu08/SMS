...
export default class EducationForm extends Component {
  ...
  schema = {
    ...
    percentage: Joi.number().required().label('course  Percentage'),
    ...
    for (let item of obj.education) {
      if (item && item.from && item !== '') {
        ...
        if (beforeInMoment.isBefore(afterInMoment)) {
          ...
          ToastService.Toast(`To value is greater then From Value`, "default")
        }
      }
    }
  }

  // dateValue = async (date, i, field) => {
  //   const data = this.formApi.getState().values;
  //   const { from, to } = date;
  //   if (from && to) {
  //     data.projectDetails[i][field] = { from: moment(from).format("YYYY-MM-DD"), to: moment(to).format("YYYY-MM-DD") };
  //     // data.projectDetails[i][field] = { from: from._d.toISOString().slice(0, 10), to: to._d.toISOString().slice(0, 10) };
