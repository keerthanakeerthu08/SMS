...
import { getFeeallocation, getfeecategory, getfeesDetails } from 'services/feeService';
...
export default class Fees extends Component {
  ...
  feildCheck = async () => {
    ...
    const { userType, userLevel, client, entity, branch, department, batch, code, branchId, departmentId, batchId, studentId, uid } = sessionData;
    data['uid'] = uid;
  ...
  clientDatas = async (name) => {
    ...
    switch (name) {
      case "client":
        ...
        await this.setState({ entity: "", branch: "", department: "", batch: "", branchIds: [], departmentIds: [], batchIds: [], feesTable: false })
        break;
      case "entity":
        await this.selectoptGet(`namelist?client=${data.client}&type=entity&entity=${data.entity}`, "branchIds")
        await this.setState({ branch: "", department: "", batch: "", departmentIds: [], batchIds: [], feesTable: false })
        break;
      case "branch":
        await this.selectoptGet(`namelist?client=${data.client}&type=branch&entity=${data.entity}&branch=${data.branch}`, "departmentIds")
        await this.setState({ department: "", batch: "", batchIds: [], feesTable: false })
      ...
      case "department":
        ...
        await this.setState({ batch: "", feesTable: false })
        break;
      case "batch":
  ...
  feecollectionList = async () => {
    const { data: { client, entity, branch, department, category, uid, studentId, userType } } = this.state;
    let params;
    params = `client=${client}&entity=${entity}&branch=${branch}&departmentId=${department}&feeid=${category}`
    if (userType === 'student')
      params = `client=${client}&entity=${entity}&branch=${branch}&departmentId=${department}&feeid=${category}&uid=${uid}`;
    if (userType === 'parent')
      params = `client=${client}&entity=${entity}&branch=${branch}&departmentId=${department}&feeid=${category}&uid=${studentId}`;
  ...
  renderFeesForm(feesType, data) {//Pass the datas to the FeeallocationList  
    const { rightsData, data: { userType } } = this.state;
    return <FeesList
      feesType={feesType}
      data={data} props={this.props} details={this.state.data}
      rightsData={rightsData} userType={userType}
    />
  }
  ...
  render() {
    let { feesType } = this.props.match.params
              ...
              {!isPageLoading && !isLoading &&
                <Fragment>
                  ...
                  <Breadcrumb>
                    ...
                    <BreadcrumbItem active>{(_form === 'feeallocation' && "Fee Allocation") || (_form === 'feecollection' && "Fee Collection") || _form}</BreadcrumbItem>
                  </Breadcrumb>
                  <Container fluid>
                    ...
                    <div className="d-md-flex align-items-md-center justify-content-md-between">
                      <h5 className="pg-title">{(_form === 'feeallocation' && "Fee Allocation") || (_form === 'feecollection' && "Fee Collection") || _form}</h5>
                      <div  >
                        {feesType !== "feecollection" ?
                          <div style={{ textAlign: 'right' }}  >
                            {rightsData && rightsData[_form] && rightsData[_form].create.value &&
                              <NavLink className="btn btn-primary btn-sm" to={`add/${feesType}`}>+ Add Fee Allocation </NavLink>
                            }
                          </div>
                          : ''}
                      </div>
