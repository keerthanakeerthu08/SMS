...
export default class FeesList extends Component {
  ...
  getColumnHeaders(type, prefixUrl = "", dynamicLabels = {}) {
    ...
    if (userType === 'student') {
      excludeKeys.unshift("studentId")
      excludeKeys.unshift("name")
    }
    ...
    let def = {
      ...
      "Fine Amount": { dataField: 'fineAmount', text: 'Fine Amount', sort: true },
      "Description": { dataField: 'description', text: 'Description', sort: true },
      "Caste": { dataField: 'caste', text: 'Caste', sort: true },
      "Excemption": { dataField: 'excemption', text: 'Excemption', sort: true },
  ...
  allocationactionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    ...
    rightsData && rightsData[_form] && rightsData[_form].delete.value &&
      links.push(<div onClick={() => confirm("Are you sure? You want to delete this", () => this.deleteRow(row))} className='badge badge-danger'>Delete</div>)
  ...
  collectionactionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    ...
    let links = []
    if ((row && row.feeCollection && row.feeCollection[0] && row.feeCollection[0].status === 'unpaid') || (row && row.feeCollection && row.feeCollection[0] && row.feeCollection[0].status === 'Pending')) {
      links.push(<div onClick={() => this.paymentFun(`/fees/add/feecollection`, row)} className='badge badge-success'>Pay</div>)
    }
    if ((row && row.feeCollection && row.feeCollection[0] && row.feeCollection[0].status === 'Paid') || (row && row.feeCollection && row.feeCollection[0] && row.feeCollection[0].status === 'paid')) {
  ...
  amountFormatter = (cell, row, rowIndex, formatExtraData) => {
    const pendingamt = parseInt(row && row.feeCollection && row.feeCollection[0] && row.feeCollection[0].amount) - parseInt(row && row.feeCollection && row.feeCollection[0] && row.feeCollection[0].paidAmount)

    if (!_.isNaN(pendingamt) && pendingamt !== 0) return pendingamt + " Pending "
    if (!_.isNaN(pendingamt) && pendingamt === 0) return "paid"
    return row && row.feeCollection && row.feeCollection[0] && row.feeCollection[0].status
