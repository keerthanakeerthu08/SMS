...
export default class FeecollectionForm extends Component {
  ...
  getSampleData = async (data, details) => {
    let studentId = data.studentId
    var test = Object.assign(data, details);
    let {feeCollection} = data;
    data.amount = feeCollection[0].amount
    data.category = feeCollection[0].category
    data.studentId = studentId;
    data.paidAmount = feeCollection[0].paidAmount
    data.remarks = feeCollection[0].remarks
   

    data.dueDate = feeCollection[0].dueDate.to;    
    var CurrentDate = new Date();
    var DueDate = new Date(feeCollection[0].dueDate.to);
    if (DueDate < CurrentDate) {
      data.paidAmount = (+feeCollection[0].amount) + (+feeCollection[0].fineAmount);
      data.fineAmount = feeCollection[0].fineAmount
    } else {
      data.paidAmount = (+feeCollection[0].amount);
      data.fineAmount = 0;
    }
  ...
  onSubmit = async (test = []) => {
    ...
    if (test.length !== 0) {
      let obj = {
        ...
        "students": [{ "uid": formdata.studentId, "paidDate": this.state.todayDate, "status": "paid", "remarks": formdata.remarks, "paidAmount": formdata.paidAmount }]
      }
                ...
                <div className="page-user">
                  <section>
                    ...
                    <Row>
                      ...
                      <Col sm={12} md={3}>
                        <Input field="fineAmount" label="Fine Amount*" name="fineAmount"
                          validateOnBlur validate={e => this.validateProperty('fineAmount', e)} onChange={this.handleChange} readOnly />
                      </Col>
                      <Col sm={12} md={3}>
                        <Input field="paidAmount" label="Paid Amount*" name="paidAmount"
                          validateOnBlur validate={e => this.validateProperty('paidAmount', e)} onChange={this.handleChange} readOnly />
                      </Col>
                    </Row>
