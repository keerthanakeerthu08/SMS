...
export default class AddAssignment extends Component {
  ...
  async  assignmentReport(data = []) {
    ...
    if (data.length !== 0) {
      ...
      if (addedDetails.data.statusCode === 1) {
        await ToastService.Toast(addedDetails.data.Message, 'default');
  ...
  async updateAssignment(data) {
    ...
    if (this.state.storeStudentMarks !== undefined) {
      ...
      let edittemp = {
        "date": this.state.todayDate,
        "updatedDate": this.state.todayDate,
        "createdDate": new Date().toISOString(),
        "batch": formdata.batch,
        "department": formdata.department,
        "assignmentId": this.state.assignmentId,
  ...
  render() {
    ...
    const { action, type, props } = this.props
            ...
            <section>
              <Row>
                <Col sm={12} md={6}>
                  <h6>Instructions</h6>
                  <div style={{ fontSize: "12px" }}>
                    <p>1. Click on marks column to edit marks
</p>
                    <p>2. Click on remarks column to enter remarks</p>
                  </div>
                </Col>
              </Row>
              <BootstrapTable
