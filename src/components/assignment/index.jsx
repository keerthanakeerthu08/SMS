...
export default class Assignment extends Component {
  ...
  onSubmit = async () => {
    ...
    if (type === 'homework') {
      ...
      const { data: { client, entity, branch, department, batch, uid, studentId, userType }, assignmentId } = this.state;
      let params;
      params = `client=${client}&entity=${entity}&branch=${branch}&department=${department}&batch=${batch}&assignmentId=${assignmentId}`;
      if (userType === 'student')
        params = `client=${client}&entity=${entity}&branch=${branch}&department=${department}&batch=${batch}&assignmentId=${assignmentId}&uid=${uid}`;
      if (userType === 'parent')
        params = `client=${client}&entity=${entity}&branch=${branch}&department=${department}&batch=${batch}&assignmentId=${assignmentId}&uid=${studentId}`;;
  ...
  renderAssignmentForm(type, data, client, entity, branch, department, batch, heading, ID) {
    ...
    return <AssignmentList type={type} data={data} props={this.props} details={details} rightsData={rightsData} userType={userType} />
  }
