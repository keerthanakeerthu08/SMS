...
export default class DashboardList extends Component {
    ...
    async componentDidMount() {
        ...
        if (data) {
            ...
            if (data.userType !== 'sadmin' && data.userType !== 'client') {

                await this.HolidayData(data)
                await this.circularGeneration(data)
            }
            if (data.userType !== 'sadmin' && data.userType !== 'client' && data.userType !== 'entity') {
    ...
    feeCollection = async (data) => {
        ...
        let today = await this.convert(new Date());
        let params, Details, collectAmount = 0, dueAmount = 0, noofTransaction = 0, totalAmount = 0;
        if (userType === 'entity') {
            params = `entity=${code}&client=${client}&date=${today}&type=dashboard`
            try {
    ...
    HolidayData = async (data) => {
        const { client, entity, branch, userType, code, branchId } = data

        if (userType)
            try {
                let params = `client=${client || code}&entity=${userType === "entity" ? code : entity}&branch=${branch || branchId}`
    ...
    AttendanceData = async (data) => {
        ...
        let { client, entity, branch, department, uid, batch } = data
        let today = new Date(), year = today.getFullYear(),
            month = today.getMonth() + 1;
        if (data.userType === "parent") uid = data.studentId
        try {
    ...
    circularGeneration = async (data) => {
        const { userType, client, entity, branch, branchId, code } = data;
        let params;
        try {
            if (userType === "entity") {
                params = `client=${client || code}&entity=${userType === "entity" ? code : entity}`;
            } else {
                params = `client=${client||code}&entity=${userType === "entity" ? code : entity}&branch=${branch || branchId}`;
            }
    ...
    render() {
        const { data, countObj, holidayData, holidayTable, coursediv, eventTable, eventData, TimetableData, TimeTable, feeCollectionDiv, studentAttendance, assignmentdiv, assignmentData } = this.state
        ...
        const piedata = {
            ...
            datasets: [{
                data: [studentAttendance && (studentAttendance[0].absent || 0), studentAttendance && (studentAttendance[0].present || 0), studentAttendance && (studentAttendance[0].total || 0)],
        ...
        return (
            <Fragment >
                ...
                <Container fluid className="bg-white ">
                    {data && data.userType !== 'staff' && data.userType !== 'sadmin' &&
                        <Row>
                            <Col sm={7}>
                    ...
                    {data && data.userType !== 'sadmin' && data.userType !== 'client' &&
                    ...
                    <div>
                        <Row>
                            <Col sm={12}>
                                ...
                                {assignmentdiv && assignmentData.length > 0 &&
                                    <section>
