...
export default class TimetableList extends Component {
    ...
    switch (form) {
      ...
      case 'workAllocation':
        ...
        if (response.data.statusCode === 1){
          ...
          await this.setState({ data: [], timeTable: true })
        }    
  ...
  async classTimeTable(Details) {
    ...
    if (Details && Details.data.statusCode === 1) {
      ...
      await this.renderTimeTable(form, finalTimeTable)
    } else {
      ToastService.Toast("No data found!!!", "default");
                      ...
                      {({ formApi, formState }) => (
                        <div>
                          <div className="d-md-flex align-items-md-center justify-content-md-between">
                            <h5 className="pg-title">{(_form === 'WorkAllocation' && "Work  Allocation") || _form}</h5>
                            {(form === 'WorkAllocation' || form === 'workAllocation') && <div style={{ textAlign: 'right' }}>
                              {rightsData && rightsData[_form] && rightsData[_form].create.value &&
                                <NavLink className="btn btn-primary btn-sm btn-right" to={`/staff/add/staffallocation`}>+ Add Work Allocation </NavLink>
                              }
                              <br />
                            </div>
                            }
                            {(form === 'class' || form === 'Class') && <div style={{ textAlign: 'right' }}>
                              {rightsData && rightsData[_form] && rightsData[_form].create.value &&
                                <NavLink className="btn btn-primary btn-sm btn-right" to={`/classLists/class`}>+ Add Class Timetable</NavLink>
                              }
                              <br />
                            </div>
                            }
