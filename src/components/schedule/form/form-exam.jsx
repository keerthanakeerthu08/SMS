...
export default class ExamForm extends Component {
  ...
  handleError = async () => {
    return ToastService.Toast("Something went wrong!!!", 'default');
  }
                ...
                <Row>
                  <Col sm={6} md={3}>
                    <CustomSelect field="term" label="Terms*" name="term" getOptionValue={option => option.id} getOptionLabel={option => option.name} validateOnBlur validate={e => this.validateProperty('term', e)}
                      options={this.state.allTerms} onChange={this.handleChange} disabled={disabled}/>
                  </Col>
                  {actiontype === 'add' &&
                    <Col sm={12} md={3}>
                      <Input
                        field="title" label="Exam Title*" name="title"
                        validateOnBlur validate={e => this.validateProperty('title', e)} disabled={disabled}
                      />
                    </Col>
                  }
                ...
                <Row>
                  <Col sm={12} md={3}>
                    <CustomSelect field="exammode" label="Exam Mode*" name="exammode" getOptionValue={option => option.id}
                      ...
                      validateOnBlur validate={e => this.validateProperty('exammode', e)} onChange={this.handleChange} disabled={disabled} />
                  </Col>
                  <Col sm={12} md={3}>
                    <Input
                      field="outoff" label="Total Marks*" name="outoff"
                      validateOnBlur validate={e => this.validateProperty('outoff', e)} disabled={disabled}
                    />
                  </Col>
                </Row>
                {exammode === 'online' &&
                  <Row>
                    ...
                    <Col sm={12} md={2}>
                      <RTimePicker
                        field="endtime" label="EndTime*" value={moment(formState.values.endtime ? formState.values.endtime : "12:00 am", "h:mm a")} onChange={(data) => this.timeValue(data, "endtime")} disabled={disabled} 
                      />
                    </Col>
                    <Col sm={12} md={2}>
                      <Input
                        field="cutoff" label="Cutoff Marks*" name="cutoff"
                        validateOnBlur validate={e => this.validateProperty('cutoff', e)} disabled={disabled}
                      />
                    </Col>
                    <Col sm={12} md={2}>
                      <Input
                        field="noOfTimes" label="No of Times*" name="noOfTimes"
                        validateOnBlur validate={e => this.validateProperty('noOfTimes', e)} disabled={disabled}
                      />
                    </Col>
                  </Row>}
              </section>
