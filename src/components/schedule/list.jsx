...
export default class ScheduleList extends Component {
  ...
  actionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    ...
    rightsData && rightsData[_form] && rightsData[_form].edit.value &&
      links.push(<div onClick={() => this.editFun(`/schedule/view/`, row)} className='badge badge-primary'>View</div>)
    rightsData && rightsData[_form] && rightsData[_form].edit.value &&
    ...
    rightsData && rightsData[_form] && rightsData[_form].delete.value &&
      row.status === 'active' &&
      links.push(<div onClick={() => confirm("Are you sure? You want to delete this", () => this.deleteScheduleTypes(row))} className='badge badge-danger'>Delete</div>)

    return <div className="actions">{links.concat(" ")}</div>
  }

  editFun = async (url, data) => {
    const { scheduleType } = this.props
