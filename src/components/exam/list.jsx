...
export default class ExamList extends Component {
  ...
  getColumnHeaders(type, prefixUrl = "", dynamicLabels = {}) {
    ...
    if (type === "viewQuestions") {
      if (data && data[0] && data[0].optionA) {


      } else {
        excludeKeys = [...excludeKeys, "QusOptionA", "QusOptionB", "QusOptionC", "QusOptionD"]
      }
    }
  ...
  offlineactionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    ...
    rightsData && rightsData[_form] && rightsData[_form].delete.value &&
      links.push(<div onClick={() => confirm("Are you sure? You want to delete this", () => this.offlineExamdeleteFun(row))} className='badge badge-danger'>Delete</div>)
  ...
  onlineactionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    ...
    if (userType === "student") {
      ...
      rightsData && rightsData[_form] && rightsData[_form].edit.value && row.status !== 'active' &&
        links.push(<div onClick={() => confirm("Are you sure? You want to delete this exam", () => this.onlineExamdeleteFun(row, 'active'))} className='badge badge-danger'>Delete</div>)
      rightsData && rightsData["section"] && rightsData["section"].view.value &&
        links.push(<div onClick={() => this.editFun(`/exam/section`, row)} className='badge badge-success'>View Section</div>)
    }
    return <div className="actions">{links.concat(" ")}</div>
  ...
  sectionactionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    ...
    rightsData && rightsData[_form] && rightsData[_form].delete.value &&
      links.push(<div onClick={() => confirm("Are you sure? You want to delete this", () => this.SessiondeleteFun(row))} className='badge badge-danger'>Delete</div>)
    rightsData && rightsData["previewQuestion"] && rightsData["previewQuestion"].view.value &&
  ...
  onlineExamAttend = async (url, row) => {
    ...
    if (count < row.noOfTimes) {
      let params = `branch=${row.branch}&client=${row.client}&departmentId=${row.departmentId}&entity=${row.entity}&examId=${row.examId}&sectionId=${row._id}`
      try {
  ...
  noofTimesCountFun = async (row) => {
    ...
    const { client, entity, branch, department, departmentId, examId } = row;
    let params = `client=${client}&entity=${entity}&branch=${branch}&department=${department || departmentId}`;
    let resultData = [];
    try {
