...
export default class AddGalaryForm extends Component {
  ...
  handleImage = async e => {
    await this.imageupload("galleryImgUrl", e.target.files[0])
  };
  ...
  imageupload = async (field, files) => {
    ...
    let galleryImgUrl = await FileUpload(files[0]).then((res) => res.data.name)
    await this.setState({ [field]: galleryImgUrl });
    await this.formApi.setValues({ ...values, [field]: galleryImgUrl });
  }
  ...
  onSubmit = async () => {
    let response;
    const { galleryImgUrl } = this.state;
    ...
    filterData[0].event[0].gallery[0]["url"] = galleryImgUrl;
