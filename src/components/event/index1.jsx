...
export default class EventRoot extends Component {
  ...
  feildCheck = async () => {
    ...
    const { userType, userLevel, client, entity, branch, code, branchId, uid, studentId } = sessionData;
    data['uid'] = uid;
  ...
  clientDatas = async (name) => {
    ...
    switch (name) {
      ...
      case "department":
        ...
        await this.selectoptGet(`scheduleTypeId?client=${data.client}&type=event&entity=${data.entity}&branch=${data.branch}`, "EventTypes")
        break;
      case "batch":
        await this.setState({})
        break;
  ...
  addeventNavigation(eventformType) { // navidate to Add Attendees Page    
    if (eventformType === "addAttendees") {
      eventformType = 'Add Attendees'
      return <NavLink onClick={this.redirectTo} className="btn btn-primary btn-sm" to={`/event/add/addAttendees `}>+ {eventformType} </NavLink>
    }
    if (eventformType === "gallery") {
      eventformType = 'Add Gallery'
      return <NavLink onClick={this.redirectTo} className="btn btn-primary btn-sm" to={`/event/add/gallery `}>+ {eventformType} </NavLink>
    }
  }
  ...
  async payloadData(data, values) {
    ...
    const { branch, client, entity, eventname, department, batch } = values
    let addParams = `client=${client}&entity=${entity}&branch=${branch}`
    await _.map(data).forEach(async (item) => {
      const { StudentName, StudentId, Fee, } = item;
      let eventBulkdata = {
      ...
      return ToastService.Toast("Uploaded Successfully!!!", 'default');
    });
  }
  ...
  render() {
    const { eventformType } = this.props.match.params
    const { isPageLoading, isLoading, isTableLoading, tableData, clientIds, entityIds, branchIds, client, entity, branch, rightsData, excludeModules, departmentIds, batchIds,
                  ...
                  <Fragment>
                    ...
                    <Container fluid>
                      ...
                      <div className="d-md-flex align-items-md-center justify-content-md-between">
                        <h5 className="pg-title">{(_form === "addAttendees" && "Add Attendees") || _form}</h5>
                        <div  >
