...
export default class MailNotification extends Component {
    ...
    clientDatas = async (name) => {
        ...
        switch (name) {
            ...
            case "template":
                ...
                formData.msg = this.state && this.state.messageType && this.state.messageType[0] && this.state.messageType[0].name;
    ...
    SGTemplates = async (type) => {
        ...
        if (config) {
            let url = 'https://api.sendgrid.com/v3/templates?generations=dynamic';
            let r = await SGTemplates(url, "GET", "", config);
            console.log(r)
            if (r.templates) {
                await this.setState({
    ...
    onSubmit = async () => {
        ...
        const { mailsenderName, accessKey, mailsender, ccemails, bccemails,messageType } = this.state
        ...
        if ((mailsenderName !== undefined) && (accessKey !== undefined) && (mailsender !== undefined)) {
            ...
            let temp = {
                ...
                "templateId": messageType[0].template_id,
                "templateArgs": '',
                "cc": cc,
                "bcc": bcc,
            }
